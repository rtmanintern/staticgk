...
function viewresult()
{
	...
	var qus=localStorage.getItem("TESTQ_G");
	var att=localStorage.getItem("TESTA_G");
	var qus_s=qus.split('/');
	var att_s=att.split('/');
	for (i=1;i<=12;i++)
	{
...
r=(r*7)%10;
r=r+13;
document.getElementById("today").innerHTML="Can You Score "+r+""+"/24";
quizstart();
myFunction1();
...
function submit()
{
	...
	s=c*2-w*0.5;
	localStorage.setItem("TESTQ_G",qus);
	localStorage.setItem("TESTA_G",att);
	document.getElementById("score").innerHTML=s;
	document.getElementById("correct").innerHTML=" "+c+" Correct";
	document.getElementById("wrong").innerHTML=" "+w+" Wrong";
	document.getElementById("skip").innerHTML=" "+ (12-(c+w)) +" Unattempt";
	var today = new Date();
var dd = String(today.getDate()).padStart(2, '0');
var mm = String(today.getMonth() + 1).padStart(2, '0'); //January is 0!
var yyyy = today.getFullYear();
...
today =  dd + '/' + mm + '/' + yyyy;
localStorage.setItem("TODAY_G",today);
var r=dd%10;
r=(r*7)%10;
r=r+13;
...
if (s>=r)
{
	...
	var m=t[0];
	var s=t[1];
	var total=(4-m)+" : "+(60-s)+" sec";
	document.getElementById("timetake").innerHTML=total;
	document.getElementById("quizcomp").style.display="block";
	var s1=c*2-w*0.5;
	var details=total+"/"+" "+c+" Correct"+ "/"+" "+ (12-(c+w)) +" Unattempt"+ "/"+" "+w+" Wrong"+"/"+""+ s1 +""+"/" +r + "/"+document.getElementById("status1").innerHTML;
	localStorage.setItem("DETAILS_G",details);
...
{
	if(localStorage.getItem("DETAILS_G")!=undefined)
	{
		var s=localStorage.getItem("DETAILS_G");
		var data=s.split('/');
